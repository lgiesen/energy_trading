...
def test_bem_guard_blocks_pos_when_locked_reserve_consumes_protected_buffer() -> None:
    bt = _mk_backtester()
    bt.afrr_bid_granularity_mw = 0.1
    bt.afrr_step_mw = 0.1
    bt.afrr_min_bid_size_mw = 0.1
    ...
    guard = bt._apply_bem_only_submission_guard(
        desired_bem_only_pos_mw=1.0,
        desired_bem_only_neg_mw=0.0,
        soc_start_mwh=bt.soc_min + 0.5,
        settlement_soc_start_mwh=bt.soc_min + 0.5,
    ...
    assert float(guard["reserve_buffer_replay_pass"]) == pytest.approx(1.0)
    assert float(guard["reserve_buffer_required_soc_min_mwh"]) == pytest.approx(bt.soc_min + 0.5)
...
def test_market_clearing_unawarded_reserve_to_bem_uses_guarded_quantity() -> None:
    bt = _mk_backtester()
    bt._strategy_permissions = StrategyPermissions(
        allow_da=False,
        allow_bcm=True,
    ...
    assert float(cleared["plan_reserve_pos_mw"]) == pytest.approx(10.0)
    assert float(cleared["bem_only_submitted_pos_mw_before_guard"]) == pytest.approx(10.0)
    assert float(cleared["bem_guard_stale_soc_anchor"]) == pytest.approx(1.0)
    assert float(cleared["bem_only_submitted_pos_mw"]) == pytest.approx(0.0)
    assert float(cleared["submitted_afrr_pos_mw"]) == pytest.approx(0.0)
    assert float(cleared["executed_reserve_pos_mw"]) == pytest.approx(0.0)
    assert str(cleared["bem_guard_zero_reason_pos"]) == "bem_guard_stale_soc_anchor"
...
def test_da_only_reserve_prewrite_guards_do_not_write_lockbooks() -> None:
    bt = _mk_backtester()
    col = BacktestColumnMap()
    da_only_perms = bt.resolve_strategy_permissions(
        strategy_name="da",
        allowed_markets=("DA",),
        id_recourse_mode="common",
    )
    bt._strategy_permissions = da_only_perms

    benign_bem = bt._apply_bem_only_submission_guard(
        desired_bem_only_pos_mw=0.0,
        desired_bem_only_neg_mw=0.0,
        soc_start_mwh=10.0,
        locked_reserve_pos_mw=0.0,
    ...
    assert float(benign_bem["submitted_bem_only_pos_mw"]) == pytest.approx(0.0)
    assert float(benign_bem["submitted_bem_only_neg_mw"]) == pytest.approx(0.0)
    assert float(benign_bem["da_only_reserve_logic_reached"]) == pytest.approx(0.0)
    assert float(benign_bem["da_only_activation_aware_guard_reached"]) == pytest.approx(0.0)

    leaking_bem = bt._apply_bem_only_submission_guard(
    ...
    assert float(leaking_bem["submitted_bem_only_pos_mw"]) == pytest.approx(0.0)
    assert float(leaking_bem["da_only_reserve_logic_reached"]) == pytest.approx(1.0)
    assert float(leaking_bem["da_only_activation_aware_guard_reached"]) == pytest.approx(1.0)
    ...
    source = pd.DataFrame({col.timestamp: target_hours}).set_index(col.timestamp)
    ...
    stats = bt._update_afrr_capacity_lockbooks_from_snapshot(
        ...
        lock_energy_pos={},
        lock_energy_neg={},
        strategy_permissions=da_only_perms,
        optimizer_allowed_markets=("DA",),
    )
    assert lock_pos == {}
    assert lock_neg == {}
    assert float(stats["triggered"]) == pytest.approx(0.0)
    assert float(stats["da_only_reserve_logic_reached"]) == pytest.approx(1.0)
    assert float(stats["da_only_reserve_lockbook_written"]) == pytest.approx(0.0)
...
def test_settlement_degradation_uses_internal_throughput_for_da_id_and_activation() -> None:
    ...
    bt.aux_off_mw = 0.0
    bt.aux_trading_mw = 0.0
    bt.id_rescue_spread_eur_mwh = 30.0
    bt.aux_standby_mw = 0.0
    bt.aux_afrr_active_mw = 0.0
    bt._strategy_permissions = StrategyPermissions(
        allow_da=True,
...
def test_da_precommit_selects_no_trade_when_predicted_replay_loses_money() -> None:
    ...
    ts_buy = pd.Timestamp("2025-01-09T00:00:00Z")
    ts_sell = pd.Timestamp("2025-01-09T01:00:00Z")
    pf_rows = pd.DataFrame(
        {
            ...
            col.pred_da_price: [200.0, 10.0],
            col.true_da_price: [10.0, 200.0],
            col.pred_afrr_activation_rate_pos: [0.0, 0.0],
            col.pred_afrr_activation_rate_neg: [0.0, 0.0],
            col.true_afrr_activation_rate_pos: [0.0, 0.0],
            col.true_afrr_activation_rate_neg: [0.0, 0.0],
            "predicted_objective_eur": [0.0, 0.0],
        }
    )
    selected_model, audit_model = bt._select_feasible_da_lock_schedule(
        lock_rows=pf_rows,
        colmap=col,
        current_soc_mwh=10.0,
        fixed_reserve_pos={},
    ...
    selected_rhpf, audit_rhpf = bt._select_feasible_da_lock_schedule(
        lock_rows=pf_rows,
        colmap=col,
        current_soc_mwh=10.0,
        fixed_reserve_pos={},
    ...
    assert selected_model[ts_buy] == pytest.approx((0.0, 0.0))
    assert selected_model[ts_sell] == pytest.approx((0.0, 0.0))
    assert selected_rhpf[ts_buy][0] > 0.0
    assert selected_rhpf[ts_sell][1] > 0.0
    assert str(audit_rhpf[0]["da_bid_sizer_method"]) == "linprog_full_window_continuous_per_hour"
    assert str(audit_rhpf[0]["da_canonical_sizer_price_source_column"]) == col.true_da_price
    assert str(audit_rhpf[0]["da_canonical_sizer_status"]) in {
        "ok",
        "ok_after_exact_soc_clip",
        "ok_hard_min_surplus_replay",
    }
...
def test_da_canonical_sizer_accepts_hard_min_surplus_terminal_replay_metadata(monkeypatch) -> None:
    ...
    def stale_terminal_shortfall_replay(**_kwargs):
        return False, {
            "first_infeasible_timestamp_utc": str(ts[-1]),
            "infeasibility_driver": "terminal_repair_missing",
            "infeasibility_driver_detail": "terminal_repair_missing",
            "projected_soc_min_mwh": 11.0,
            "projected_soc_max_mwh": 12.0,
            "projected_final_soc_mwh": 12.0,
        }

    monkeypatch.setattr(bt, "_project_da_postlock_future_feasibility", stale_terminal_shortfall_replay)
    ...
    assert {str(row["da_bid_sizer_status"]) for row in audit} == {"ok_hard_min_surplus_replay"}
    assert {str(row["da_canonical_sizer_status"]) for row in audit} == {"ok_hard_min_surplus_replay"}
    assert {str(row["da_canonical_sizer_failure_reason"]) for row in audit} == {"none"}
    assert {float(row["da_canonical_sizer_rounded_replay_pass"]) for row in audit} == {1.0}


def test_da_canonical_sizer_does_not_hide_hard_min_soc_bound_failure(monkeypatch) -> None:
    bt = _mk_backtester()
    col = BacktestColumnMap()
    bt.final_soc_mode = "hard_min"
    bt.eta_in = 1.0
    ...
    def soc_bound_failure_replay(**_kwargs):
        return False, {
            "first_infeasible_timestamp_utc": str(ts[-1]),
            "infeasibility_driver": "terminal_repair_missing",
            "infeasibility_driver_detail": "terminal_repair_missing",
            "projected_soc_min_mwh": 11.0,
            "projected_soc_max_mwh": 18.5,
            "projected_final_soc_mwh": 12.0,
        }

    monkeypatch.setattr(bt, "_project_da_postlock_future_feasibility", soc_bound_failure_replay)
    ...
    assert sum(ch + dis for ch, dis in selected.values()) == pytest.approx(0.0)
    assert {str(row["da_canonical_sizer_status"]) for row in audit} == {"da_sizer_replay_failed"}
    assert {str(row["da_canonical_sizer_failure_reason"]) for row in audit} == {"terminal_repair_missing"}
    assert {float(row["da_canonical_sizer_rounded_replay_pass"]) for row in audit} == {0.0}
...
def test_bem_only_can_coexist_with_bcm_linked_activation_source_split() -> None:
    ...
    assert float(out["bem_free_volume_mw"]) == pytest.approx(2.0)
    assert float(out["bem_total_bid_volume_mw"]) == pytest.approx(7.0)
    assert float(out["bem_only_submitted_pos_mw"]) == pytest.approx(2.0)
    assert float(out["bem_only_pos_reduced_by_headroom_mw"]) == pytest.approx(1.0)
    assert float(out["bcm_linked_pos_activation_mwh"]) == pytest.approx(5.0)
    assert float(out["bem_only_pos_activation_mwh"]) == pytest.approx(2.0)
    assert float(out["bem_pos_activation_mwh"]) == pytest.approx(7.0)
    ...
    assert float(comp["bem_only_activation_revenue_eur"]) == pytest.approx(200.0)
    assert float(out["bem_activation_revenue_eur"]) == pytest.approx(
...
def test_bem_only_guard_uses_pre_delivery_settlement_soc_for_protected_neg() -> None:
    bt = _mk_backtester()
    bt.bem_activation_headroom_h = 0.5
    bt.bem_only_headroom_safety_mwh = 0.0
    bt.reserve_headroom_safety_mwh = 0.0
    ...
    guard = bt._apply_bem_only_submission_guard(
        ...
        desired_bem_only_neg_mw=1.0,
        soc_start_mwh=12.25,
        settlement_soc_start_mwh=18.0,
        locked_reserve_pos_mw=0.0,
        locked_reserve_neg_mw=1.0,
        pred_act_pos=0.0,
        pred_act_neg=0.0,
        activation_rate_pos=0.0,
        activation_rate_neg=0.5,
        da_charge_mw=0.0,
        da_discharge_mw=0.0,
        id_charge_mw=0.0,
        id_discharge_mw=0.0,
    ...
    assert float(guard["bem_only_guard_soc_now_mwh"]) == pytest.approx(18.0)
    assert float(guard["settlement_soc_start_mwh"]) == pytest.approx(18.0)
    assert float(guard["submitted_bem_only_neg_mw"]) == pytest.approx(0.0)
    assert str(guard["bem_only_headroom_guard_reason"]) in {
        "protected_soc_prevented_neg",
...
def test_da_postlock_nonfinal_terminal_shortfall_is_not_labeled_recoverable() -> None:
    ...
    future_rows = pd.DataFrame(
        {
            col.timestamp: ts,
            col.pred_da_price: [1_000.0, 0.0, 0.0],
            col.pred_afrr_activation_rate_pos: [0.0, 0.0, 0.0],
            col.pred_afrr_activation_rate_neg: [0.0, 0.0, 0.0],
        }
    )
...
def test_da_postlock_final_terminal_shortfall_without_repair_is_explicit_infeasible() -> None:
    bt = _mk_backtester()
    bt.da_postlock_guard_mode = "recoverability_aware"
    bt.final_soc_mode = "hard_min"
    bt.soc_target_end = 10.0
...
def test_da_postlock_terminal_shortfall_unrecoverable_rejected() -> None:
    ...
    assert str(audit[0]["da_postlock_infeasibility_driver"]) == "terminal_repair_missing"
...
def test_da_postlock_terminal_recovery_cost_can_select_no_trade() -> None:
    ...
    assert selected[ts] == pytest.approx((0.0, 0.0))
    assert str(audit[0]["da_postlock_selected_after_future_check"]) == "rejected"
    assert str(audit[0]["da_zero_reason"]) in {"terminal_repair_missing", "postlock_replay_infeasible"}
    assert float(audit[0]["da_postlock_terminal_shortfall_unrecoverable"]) == pytest.approx(1.0)
    assert str(audit[0]["final_selected_incumbent"]) == "no_trade"
...
def test_da_postlock_terminal_recovery_cost_keeps_better_candidate_than_no_trade() -> None:
    bt = _mk_backtester()
    bt.da_postlock_guard_mode = "recoverability_aware"
    bt.final_soc_mode = "hard"
    bt.eta_in = 1.0
    ...
    col = BacktestColumnMap()
    ts = pd.Timestamp("2026-01-02T00:00:00Z")
    final_ts = ts + pd.Timedelta(hours=1)
    rows = pd.DataFrame(
        {
            col.timestamp: [ts, final_ts],
            "target_time_utc": [ts, final_ts],
            "snapshot_time_utc": [
                pd.Timestamp("2026-01-01T10:00:00Z"),
                pd.Timestamp("2026-01-01T10:00:00Z"),
            ],
            "soc_start_lp_mwh": [8.0, 8.0],
            col.pred_da_price: [300.0, 100.0],
            col.pred_afrr_capacity_price_pos: [0.0, 0.0],
            col.pred_afrr_capacity_price_neg: [0.0, 0.0],
            col.pred_afrr_activation_price_pos: [0.0, 0.0],
            col.pred_afrr_activation_price_neg: [0.0, 0.0],
    ...
    selected, audit = bt._apply_da_postlock_future_guard(
        selected_da={final_ts: (0.5, 0.0)},
        da_audit_rows=[
            {
                "timestamp_utc": str(final_ts),
                "da_candidate_buy_mw": 0.5,
                "da_candidate_sell_mw": 0.0,
            }
        ],
        lock_rows=rows,
        ...
        fixed_reserve_pos={},
        fixed_reserve_neg={},
        global_end_utc=final_ts,
    )

    assert selected[final_ts] == pytest.approx((0.5, 0.0))
    assert str(audit[0]["da_postlock_selected_after_future_check"]) == (
        "candidate_terminal_recovery_costed_symmetrically"
    )
    assert float(audit[0]["candidate_selection_pnl_eur_after_terminal_recovery"]) > float(
        audit[0]["incumbent_selection_pnl_eur_after_terminal_recovery"]
    )
...
def test_da_delivery_plan_executes_only_lockbook_quantities_with_origin() -> None:
    ...
    missing_quantile_clearing = bt._apply_market_clearing(
        target_time_utc=ts_locked,
    ...
    assert float(missing_quantile_clearing["submitted_da_buy_mw"]) == pytest.approx(1.5)
    assert str(missing_quantile_clearing["da_limit_price_source_column"]) == f"{col.pred_da_price}_p90"
    assert float(missing_quantile_clearing["da_price_fallback_used"]) == pytest.approx(0.0)
...
def test_rhpf_da_sizer_fallback_invalidates_upper_bound_and_explains_naive_win() -> None:
    ...
    assert str(diag["naive_beats_rhpf_reason"]) == "rolling_pf_da_sizer_failed"
    ...
    assert str(diag["rhpf_benchmark_quality_reason"]) == "rolling_pf_da_sizer_failed"
...
def test_da_rhpf_sizer_fallback_invalidates_rhpf_benchmark_only() -> None:
    ...
    assert "rhpf_da_sizer_failed" in str(out["rhpf_invalid_reason"])
    assert float(out["rhpf_benchmark_quality_pass"]) == pytest.approx(0.0)
    assert str(out["rhpf_benchmark_quality_reason"]) == "rhpf_da_sizer_failed"
