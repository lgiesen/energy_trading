...
- **Primär-Target:** `target_afrr_activation_price_vwap_pos_h1`.  
...
| Zielvariablen/Outcome-Spalten (aus `X` ausgeschlossen) | `target_afrr_activation_price_vwap_pos_h1`, `target_da_price_h1`, `target_afrr_rate_h1`, `target_afrr_capacity_price_pos_h1`, `target_afrr_capacity_price_neg_h1` |

Hinweis: `target_afrr_capacity_price_pos_h1` und
`target_afrr_capacity_price_neg_h1` sind reine **Label-Spalten** (`y`) und
...
| **PICASSO-Regime-Flags** (`is_picasso_active`, `is_picasso_regime`, `market_regime_picasso`) | Abbildung zweier Regime-Anker: struktureller Schnitt im Datensatz (`2022-06-22 22:00 UTC`) und aktive PICASSO-Marktphase (`ab Juli 2024`). | Ermöglicht dem Modell die Trennung zwischen Übergangsregime und voll gekoppelter europäischer Preissetzungslogik.          |
...
| **Strict Target Policy** (`target_*_h1`)                                                      | Unverschobene Outcome-Reihen würden Nowcasting statt Forecasting erzwingen.                                                                  | Trennung von `X` und `y` entlang der Zeitachse; Prognoseziel bleibt strikt `t+1`.                                          |
...
| `afrr_activation_rate`, `is_activated`, `mfrr_active_lag`                                                  | `1h, 2h, 3h, 6h, 12h, 24h`         | Aktivierungsintensität zeigt ausgeprägte Intraday-Momentum-Cluster.                             |
...
| `afrr_activation_rate` (1h, 2h, 3h, 6h, 12h, 24h)                                                                                                                                    | Anteil (0-1) | Verhältnis aktivierter zu vorgehaltener aFRR-Leistung als Intensitätsmaß der Systembeanspruchung.                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `is_weekend`, `is_afternoon`, `is_evening`, `is_morning`, `is_night`, `is_bridge_day`, `is_payday_period`, `is_christmas_break`, `is_picasso_active`, `is_picasso_regime` (ohne Lag) | Flag (0/1)   | Binäre Regimeindikatoren für kalender- und marktstrukturbedingte Nachfragemuster und Aktivierungswahrscheinlichkeiten.                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Strict Target Policy**          | Für Training sind ausschließlich `target_*_h1` als Zielvariablen zulässig; unshifted Ziel-nahe Reihen dienen nur Audit/`y_true`. | Stellt sicher, dass Forecasting (h+1) nicht in Nowcasting kippt.                |
