...
\begin{align}
    ...
    \Pi^{\mathrm{perfect_foresight}} & : \text{perfect-foresight upper bound using realized prices/activations in planning}.
\end{align}
The perfect_foresight is interpreted as an informational upper bound, not as an implementable trading policy.
...
To normalize across periods with different market scales, profit capture is evaluated relative to benchmark and perfect_foresight envelopes:
\begin{equation}
    CR^{\mathrm{bench}}_{m} = \frac{\Pi^{\mathrm{real}}_{m}}{\Pi^{\mathrm{real}}_{\mathrm{bench}}},
    \qquad
    CR^{\mathrm{perfect_foresight}}_{m} = \frac{\Pi^{\mathrm{real}}_{m}}{\Pi^{\mathrm{real}}_{\mathrm{perfect_foresight}}}.
\end{equation}
$CR^{\mathrm{perfect_foresight}}_{m}\in(-\infty,1]$ measures proximity to the informational upper bound.
