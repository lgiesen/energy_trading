...
def add_confidence_features(df: pl.DataFrame) -> pl.DataFrame:
    ...
    pairs = {
        "solar": ("solar_forecast", "solar_actual"),
        "wind_onshore": ("wind_onshore_forecast", "wind_onshore_actual"),
    ...
    if "solar_forecast" in df.columns:
        df = df.with_columns(
            (pl.col("solar_forecast") - pl.col("solar_forecast").shift(1)).abs().alias("solar_ramp")
        )
    if "wind_onshore_forecast" in df.columns:
        df = df.with_columns(
            (pl.col("wind_onshore_forecast") - pl.col("wind_onshore_forecast").shift(1)).abs().alias("wind_ramp")
        )
    if {"wind_onshore_forecast_intraday", "wind_onshore_forecast"}.issubset(df.columns):
        df = df.with_columns(
            (pl.col("wind_onshore_forecast_intraday") - pl.col("wind_onshore_forecast"))
            .abs()
            .alias("wind_onshore_delta_fc")
        )
    if {"load_forecast_da", "wind_onshore_forecast", "solar_forecast"}.issubset(df.columns):
        df = df.with_columns(
            (pl.col("load_forecast_da") - pl.col("wind_onshore_forecast") - pl.col("solar_forecast"))
